...
Bool parseLine(char *token, char *line)
{
    ...
    else if (isInstruction(token))
    {
        ...
        {
            if (type == _TYPE_DATA)
                yieldWarning(emptyDataDeclaretion);
        ...
        {
            if ((*globalState)() == firstRun)
                isValid = handleInstruction(type, token, next, line) && isValid;
            else
            {
                if (type == _TYPE_DATA)
                    isValid = writeDataInstruction(next) && isValid;
                else if (type == _TYPE_STRING)
                    isValid = writeStringInstruction(next) && isValid;
                else
                    return True;
            }
        }
